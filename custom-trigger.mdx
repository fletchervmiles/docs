...
Here are some common ways to trigger the modal:

| Use-case | How |
|----------|-----|
| Open from a navbar link | `<a href="#" onclick="FrankoModal.open()">Chat</a>` |
| One page, two modals | Install two snippets with different `data-modal-slug` values and call `FrankoModal.open('other-slug')` |

**Examples**

- **Open from a navbar link**: `<a href="#" onclick="FrankoModal.open()">Chat</a>`
- **Multiple modals**: Install two snippets with different `data-modal-slug` values and call `FrankoModal.open('other-slug')`
...
**Will unused scripts hurt performance?** No – each script is less than 5 kB and loads async.

**Does the script inject a bubble?** No. Manual mode stays invisible until you call `FrankoModal.open()`.

Need more? Email **fletcher@franko.ai**. 